...
def Lasso_fit(X, y, _lambda=0.1):
    ''' A function that fits a model using Lasso regression.
...
def k_fold_cross_validation(X, z, k=10, fit_type=OLS_fit, predict_type= OLS_predict, tradeoff = False, **parameters):
    ''' A function that predicts a value using linear regression.
        ...
        Args:
            ...
            tradeoff (boolean): decided whether or not to calculate bias and variance


        Returns:
            tot_err_test (float): average MSE from test data after k folds
            tot_err_train (float): average MSE from train data after k fold
    ...
    # if we want the bias-variance tradeoff
    # this part of the code does not yield the expected results, use bootstrap instead
    else:
...
# code inspired by Morten's code
def bootstrap(X, z, num_bootstraps, fit_type=OLS_fit, predict_type= OLS_predict, **parameters):
    ''' A function that calculates the MSE, bias and variance using the bootstrap method.

        Args:
            X (matrix): design matrix
            z (array): true values
            num_bootstraps(int or float): number of bootstrap iterations
            fit_type (function): which regression fit to use
            predict_type (function): which regression to use for prediction

        Returns:
            error_test (float): the average test MSE after num_bootstraps iterations
            error_train (float): the average train MSE after num_bootstraps iterations
            bias (float): the average bias after num_bootstraps iterations
            variance (float): the average variance after num_bootstraps iterations
    '''
